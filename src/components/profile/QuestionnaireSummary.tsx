...
import { ageFromBirthDate, formatBirthDate } from "@/lib/birthdate";
  ...
  return (
    <div className="flex items-start gap-3.5 py-4 first:pt-0 last:pb-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className={value ? "font-display text-lg leading-snug" : "text-sm text-muted mt-0.5"}>{value ?? t("notSet")}</div>
...
export function QuestionnaireSummary({
  ...
  const genderValue = gender === "female" || gender === "male" ? t(`gender.${gender}`) : null;
  ...
  return (
    <div className="flex flex-col divide-y divide-border">
      <Row
        icon={CalendarHeart}
        label={t("birthDate")}
        value={birthValue ? (age !== null ? `${birthValue} · ${t("years", { count: age })}` : birthValue) : null}
      />
      <Row icon={UserRound} label={t("genderLabel")} value={genderValue} />
      <Row
        icon={Sparkles}
        label={t("face")}
        value={skinTypeLabel(tSkin, skinType)}
        concerns={skinConcerns.map((c) => skinConcernLabel(tSkin, c))}
        concernsLabel={t("concerned")}
      />
      <Row
        icon={Scissors}
        label={t("hair")}
        value={hairTypeLabel(tHair, hairType)}
        concerns={hairConcerns.map((c) => hairConcernLabel(tHair, c))}
        concernsLabel={t("concerned")}
      />
    </div>
  );
